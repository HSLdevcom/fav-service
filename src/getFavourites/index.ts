...
import { RedisSettings, Cache, Favourite, Favourites, GetSchema } from '../util/types';
...
const getFavoritesTrigger: AzureFunction = async function (context: Context, req: HttpRequest): Promise<void> {
  const cache: Cache = {};
  ...
  const key = store ? `${store}-${userId}` : userId;
  const data: string | null = await client.get(String(key));
  ...
  try {
    ...
    if (cache.data === null) {
      ...
      context.log('found datastorage', dataStorage);
